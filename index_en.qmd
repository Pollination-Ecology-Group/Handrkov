---
title: "Pollinators of Handrkov" 
lang: en
website:
  navbar:
    background: rgb(64, 105, 91)
    logo-alt: "Pollinators of Handrkov"
    left:
      - href: index_en.qmd
        text: "Pollinators of Handrkov"
      - href: index_en.qmd#mission
        text: "Mission & Goals" 
      - href: index_en.qmd#practical-info
        text: "Practical Info"     
      - href: index_en.qmd#join-us
        text: "Join Us!"         
      - href: index_en.qmd#contact
        text: "Contact"
    right:
      - text: "CZ"
        href: index.qmd
      - text: "EN"
        href: index_en.qmd
      - icon: github
        href: https://github.com/Pollination-Ecology-Group
      - icon: whatsapp
        href: https://wa.me/420774553734
      - icon: instagram
        href: https://www.instagram.com/opylovaci_handrkov_/
      - icon: spotify
        href: https://open.spotify.com/show/3ypxSzPMkd4TOepn3oTrVl
      - icon: youtube
        href: https://www.youtube.com/@opylovaci_podcast
...
<div class="h1" style="text-align: center; color: #354e6b; font-weight: bold; margin-bottom: 0.5rem; line-height: 1.2;">Handrkov Pollination Field Research</div>
...
[Relationship between plants and pollinators in the meadow (Article in Živa magazine - CZ)](https://ziva.avcr.cz/files/ziva/pdf/vztahy-rostlin-a-opylovacu-na-louce-aneb-nejen-bot.pdf)
...
<script>
  function translateNavbar() {
    ...
    // We check both HREF and TEXT content for robustness
    const translations = [
      // Home
      { 
        matchText: "Opylovači Handrkov", 
        matchHref: "index.qmd", // From _quarto.yml
        newText: "Handrkov Pollinators", 
        newHref: "index_en.html" 
      },
      // Mission
      { 
        matchText: "Smysl a cíle",
        matchHref: "#mise",
        newText: "Mission and Goals", 
        newHref: "#mission" 
      },
      // Practical Info
      { 
        matchText: "Praktické informace",
        matchHref: "#prakticke-informace",
        newText: "Practical Information", 
        newHref: "#practical-info" 
      },
      // Join Us
      { 
        matchText: "Přidej se!",
    ...
    navLinks.forEach(link => {
      ...
      for (const t of translations) {
        // Match logic:
        // 1. Exact text match (ignoring case usually good, but here specific)
        // 2. HREF ends with the key (to handle relative/absolute issues)
        const textMatch = currentText === t.matchText;
        const hrefMatch = currentHref && (currentHref === t.matchHref || currentHref.endsWith(t.matchHref.replace('index.qmd', 'index.html')));

        if (textMatch || hrefMatch) {
            // Only update if not already updated
            if (link.textContent.trim() !== t.newText) {
                link.textContent = t.newText;
                link.setAttribute('href', t.newHref);
            }
            break; // Stop checking this link once matched
        }
      }
    });
    ...
    if (brand) {
         // Update href
         const currentBrandHref = brand.getAttribute('href');
         if (currentBrandHref && (currentBrandHref.endsWith('index.html') || currentBrandHref.endsWith('index.qmd') || currentBrandHref === '/')) {
             brand.setAttribute('href', 'index_en.html');
         }

         // Update Title
         const title = brand.querySelector('.navbar-title');
         if (title) {
             const titleText = title.textContent.trim();
             if (titleText === "Opylovači Handrkov") {
                 title.textContent = "Handrkov Pollinators";
             }
         }
    }
  }

  // Running immediately, on load, and with interval/observer for robustness
  translateNavbar();
  ...
  window.addEventListener('load', translateNavbar);

  // Poll for a few seconds to catch delayed rendering
  const intervalId = setInterval(translateNavbar, 100);
