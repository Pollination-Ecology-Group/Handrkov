...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const translations = {
      "Opylovači Handrkov": { text: "Handrkov Pollinators", href: "index_en.html" },
      "Smysl a cíle": { text: "Mission and Goals", href: "#mission" },
      "Praktické informace": { text: "Practical Information", href: "#practical-info" },
      "Přidej se!": { text: "Join Us!", href: "#join-us" },
      "Kontakt": { text: "Contact", href: "#contact" }
    };

    navLinks.forEach(link => {
      const linkText = link.textContent.trim();
      if (translations[linkText]) {
        link.textContent = translations[linkText].text;
        link.setAttribute('href', translations[linkText].href);
      }
    });
    
    // Also update the brand link if it exists and points to index.qmd/html
    const brand = document.querySelector('.navbar-brand');
    if (brand) {
        brand.setAttribute('href', 'index_en.html');
        // If there is a title span
        const title = brand.querySelector('.navbar-title');
        if (title && title.textContent.trim() === "Opylovači Handrkov") {
